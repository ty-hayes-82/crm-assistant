"""
Base agent classes and utilities for CRM multi-agent system.
Provides common functionality and patterns for all CRM agents.
"""
...
class BaseAgent(LlmAgent):
    """
    Base class for all CRM agents with common MCP toolset configuration.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize a CRM agent with standard MCP toolset.
        ...
        """
        # Standard MCP toolset for all CRM agents (with warning suppression)
...
class SpecializedAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        description = f"Specialized agent for {domain} operations in CRM data"
        
        # Use provided instruction or build default based on domain and tools
        if instruction is None:
            tools_list = ", ".join(specialized_tools)
            instruction = f"""You are a specialized CRM {domain} agent. 
Your primary responsibility is {domain} operations on CRM data.
You have access to these specialized tools: {tools_list}

Always:
1. Load the CRM data first if not already loaded
